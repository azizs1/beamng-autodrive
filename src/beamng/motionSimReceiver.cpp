...
MotionSimReceiver::MotionSimReceiver(int port) 
    : port(port), sockfd(-1) {}

MotionSimReceiver::~MotionSimReceiver() {
    if (sockfd >= 0) { 
        close(sockfd); 
    }
}
    
bool MotionSimReceiver::init() {
    ...
    std::cout << "Successfully binded MotionSim, port " << port << std::endl;

    return true;
}

bool MotionSimReceiver::poll() {
    ssize_t recv_bytes = recv(sockfd, &last_packet, sizeof(MotionSimReceiver), 0);

    if (recv_bytes == sizeof(MotionSimReceiver)) {
        return true;
    }
    return false;
}

const MotionSimPacket& MotionSimReceiver::getLastPacket() const {
    return last_packet;
}